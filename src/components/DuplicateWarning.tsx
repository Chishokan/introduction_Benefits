...
export function DuplicateWarning({ duplicates }: { duplicates: DuplicateMatch[] }) {
  return (
    <div role="alert" className="space-y-2 rounded-lg border border-rose-300 bg-rose-50 p-3 text-sm text-rose-800">
      ...
      <ul className="space-y-1">
        {duplicates.map((d) => (
          <li key={d.id}>
            <Link href={`/admin/referrals/${d.id}`} target="_blank" className="font-mono font-semibold underline">
              {d.code}
            </Link>
